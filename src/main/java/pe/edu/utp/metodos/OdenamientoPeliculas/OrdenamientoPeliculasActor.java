...
public class OrdenamientoPeliculasActor {
    ...
    private static void merge(Pelicula[] resultado, Pelicula[] izquierda, Pelicula[] derecha) {
        ...
        while (i < izquierda.length && j < derecha.length) {
            if (izquierda[i].getActor().compareTo(derecha[j].getActor()) <= 0) {
    ...
    public static int busquedaBinary(Pelicula[] peliculas, String nombre) {
        ...
        while (izquierda <= derecha) {
            ...
            if (peliculaEnMedio.getActor().equals(nombre)) {
                return medio;
            } else if (peliculaEnMedio.getActor().compareTo(nombre) < 0) {
                izquierda = medio + 1;
            } else {
                derecha = medio - 1;
            }
