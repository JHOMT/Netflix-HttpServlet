...
public class OrdenamientoPeliculasNombre {
    ...
    private static void merge(Pelicula[] resultado, Pelicula[] izquierda, Pelicula[] derecha) {
        ...
        while (i < izquierda.length && j < derecha.length) {
            if (izquierda[i].getName().compareTo(derecha[j].getName()) <= 0) {
    ...
    public static int busquedaBinary(Pelicula[] peliculas, String nombre) {
        ...
        while (izquierda <= derecha) {
            ...
            if (peliculaEnMedio.getName().equals(nombre)) {
                return medio;
            } else if (peliculaEnMedio.getName().compareTo(nombre) < 0) {
                izquierda = medio + 1;
            } else {
                derecha = medio - 1;
            }
